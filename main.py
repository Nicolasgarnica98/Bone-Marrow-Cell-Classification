...
def main():
    ...
    if os.path.exists('./dataset/train') == False:
        ...
        num_item_per_class = get_dataset.count_classes(df_img=df_img,df_labels= df_lbl)
        if num_item_per_class[0] > 0:
            df_img = get_dataset.delete_unbalanced_classes(df_img=df_img,df_labels=df_lbl)
            df_img = glob.glob(os.path.join('dataset/bone_marrow_cell_dataset/*.jpg'))
        
        #Balncing classes
        df_labels = get_dataset.get_labels(df_img, df_lbl)
        #Split dataset
        df_img_train, df_img_test, df_lbl_train, df_lbl_test = train_test_split(df_img,df_labels,test_size=0.1)
    ...
    def train_pipeline(selected_model=None):
        ...
        lbl_train = get_dataset.get_labels(df_img=df_train, data='df_img')[0]
        print(get_dataset.count_classes(df_img=df_train, df_labels=df_lbl))

        print('\n Val')
        df_val = glob.glob(os.path.join('dataset/val/*.jpg'))
        lbl_val = get_dataset.get_labels(df_img=df_val, data='df_img')[0]
        print(get_dataset.count_classes(df_img=df_val, df_labels=df_lbl))
        print('\n')
    ...
    get_dataset.count_classes(df_img=df_test, df_labels=df_lbl)
    ...
    lbl_test, df_lbl = get_dataset.get_labels(df_img=df_test, data='df_img')
